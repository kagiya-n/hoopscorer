...
- 進行中の試合は `localStorage` キー `hoops_tracker_v1` に `AppState` 全体（teams / 項目設定 / `events[]` / `focus` 等）を保存。
- 過去試合は別キー `hoops_tracker_games_v1` に配列で保存。**「💾 保存して新規」** ボタンから明示的にアーカイブ（任意ラベル入力可、空なら自動命名）。リセットは履歴に残さず破棄。
- **「📋 共有」** ボタンと、設定パネル「過去の試合」リストから、テキスト形式 / Markdown 表形式の2種でクリップボードへコピー可能。
  - テキスト：LINE / Slack に貼り付け向け（PTS と項目別を等幅で整列）
  - Markdown 表：GitHub / Notion 向け（PTS 以降は右寄せ）
- 履歴の「読み込み」（再編集）は意図的に未対応：誤操作で実績を上書きしてしまうリスクを避けるため、コピーと削除のみ。
...
- **データ書き出し**：CSV エクスポートも追加余地あり（現状はテキスト／Markdown のみ）。
